// Story data — the single source of truth for the game's branching narrative.
// Loaded via a <script> tag (no fetch), so it works as pure static files on
// file:// and on GitHub Pages without any server.
//
// Each node: { id, sceneText, imagePrompt, choices: [{ text, nextId }] }.
// The image for a node is loaded from assets/<id>.webp.
const STORY_DATA = [
  {
    id: "start",
    sceneText:
